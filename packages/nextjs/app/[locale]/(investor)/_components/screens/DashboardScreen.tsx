...
export function DashboardScreen() {
  ...
  return (
    ...
    >
      {/* Portfolio Hero */}
      <motion.div variants={itemVariants} className="py-3 text-center">
      ...
      {/* Summary Grid (2x2) */}
      <motion.div variants={itemVariants} className="grid grid-cols-2 gap-2.5 py-4">
        <SummaryCard
      ...
      {summary.lots.length > 0 && (
        <motion.div variants={itemVariants} className="pb-3">
          <h2 className="pb-3 font-playfair text-lg font-bold text-vaca-neutral-gray-900">
            {t("performance.title")}
          </h2>
      ...
      {/* TODO: Upcoming Events section — re-enable when event data is ready */}
    </motion.div>
  );
}
...
function SummaryCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-vaca-neutral-gray-100 bg-vaca-neutral-white px-3.5 py-3">
      <p className="font-inter text-[10px] font-medium uppercase tracking-wider text-vaca-neutral-gray-400">
        {label}
      </p>
      <p className="mt-1 font-inter text-base font-semibold tabular-nums text-vaca-neutral-gray-900">
        {value}
      </p>
    </div>
  );
...
function DashboardSkeleton() {
  return (
    <div className="mx-auto max-w-md animate-pulse px-4">
      <div className="mb-3 h-10 w-36 rounded-lg bg-vaca-neutral-gray-100" />
      <div className="mb-2 h-4 w-28 rounded bg-vaca-neutral-gray-100" />
      <div className="mb-4 h-32 rounded-2xl bg-vaca-neutral-gray-100" />
      <div className="mb-4 grid grid-cols-2 gap-2.5">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-16 rounded-2xl bg-vaca-neutral-gray-100" />
        ))}
      </div>
      <div className="h-20 rounded-2xl bg-vaca-neutral-gray-100" />
    </div>
  );
}
